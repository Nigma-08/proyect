...
public class Graph
{
     ...
     * @param 
     */
    public Graph(int nations ,int[][] routes , int[][] armies){
        this(500,500);
        int pos;
    ...
    /**
     * Hace una copia del estado actual del objeto 
     */
    public GraphState createMemento(){
        GraphState copyGraph = new GraphState();
        copyGraph.addLengthAndWidth(length,width);
        copyGraph.addFondo(fondo);
        copyGraph.addNations(nations);
        copyGraph.okAndPaymentAndisVisible(ok,payment,isVisible);
        return copyGraph;
    }
    
    /**
     * Restaura el obejto dado una copia del GraphState
     * @param state Estado GraphState
     */
    public void restore(GraphState state){
        makeInvisible();
        length = state.getLength();
        width = state.getwidth();
        fondo = state.getFondo();
        nations = state.getNations();
        ok = state.getOk();
        payment = state.getPayment();
        isVisible = state.getIsVisible();
        if(isVisible){
            makeVisible();
        }
    }
